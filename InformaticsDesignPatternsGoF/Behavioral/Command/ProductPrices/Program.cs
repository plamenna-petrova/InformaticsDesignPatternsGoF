...
namespace ProductPrices
{
    ...
    public class PriceModificator
    {
        private readonly List<ICommand> commands;

        private ICommand command;

        public PriceModificator()
        {
    ...
    public class Program
    {
        ...
        {
            var priceModificator = new PriceModificator();

            var product = new Product("Phone", 500);

            Execute(priceModificator, new ProductCommand(product, PriceAction.Increase, 100));

            Execute(priceModificator, new ProductCommand(product, PriceAction.Increase, 50));

            Execute(priceModificator, new ProductCommand(product, PriceAction.Decrease, 25));

            Console.WriteLine(product);
            Console.WriteLine();

            priceModificator.UndoActions();

            Console.WriteLine(product);
        }

        private static void Execute(PriceModificator priceModificator, ICommand productCommand)
        {
            priceModificator.SetCommand(productCommand);
            priceModificator.Invoke();
        }
    }
}
